...
def bql_fetch_price_data(tickers_dict, data_type="Price"):
    ...
    # 市場價格 Fallback 預設值
    # ==========================================================================
    # 資料來源: CNBC, Yahoo Finance, FRED, Trading Economics
    # 最後更新: 2026年1月24日
    #
    # 說明: 當無法連接 Bloomberg BQL 時使用這些預設值
    #       在 BQNT 環境中會使用即時 Bloomberg 數據
    #       這些數值會加上 ±2% 的隨機變動以模擬市場波動
    #
    # 更新方式: 請定期檢查並更新這些數值以保持準確性
    # ==========================================================================
    FALLBACK_VALUES = {
        # ----------------------------------------------------------------------
        # 全球股票指數 (Global Stock Indices)
        # 來源: Yahoo Finance, CNBC
        # ----------------------------------------------------------------------
        'S&P 500': 6960,        # S&P 500 指數 (2026/01/24 開盤: 6,960.54)
        'Dow Jones': 49200,     # 道瓊工業指數 (2026/01/12: 49,191.99)
        'NASDAQ': 22800,        # 納斯達克綜合指數
        'Russell 2000': 2320,   # 羅素2000小型股指數
        'DAX': 21500,           # 德國DAX指數
        'FTSE 100': 8600,       # 英國富時100指數
        'CAC 40': 7950,         # 法國CAC 40指數
        'Nikkei 225': 40200,    # 日經225指數
        'Shanghai': 3280,       # 上證綜合指數
        'Hang Seng': 19800,     # 香港恆生指數
        'TAIEX': 23500,         # 台灣加權指數
        'KOSPI': 2560,          # 韓國綜合股價指數

        # ----------------------------------------------------------------------
        # 全球10年期公債殖利率 (Global 10Y Bond Yields) - 單位: %
        # 來源: FRED, U.S. Treasury
        # ----------------------------------------------------------------------
        'US 10Y': 4.24,         # 美國10年期公債殖利率
        'Germany 10Y': 2.48,    # 德國10年期公債殖利率
        'UK 10Y': 4.60,         # 英國10年期公債殖利率
        'Japan 10Y': 1.20,      # 日本10年期公債殖利率
        'China 10Y': 1.70,      # 中國10年期公債殖利率
        'France 10Y': 3.30,     # 法國10年期公債殖利率

        # ----------------------------------------------------------------------
        # 美股產業ETF (US Sector ETFs) - 單位: USD
        # 來源: Yahoo Finance (XLK, XLF, XLV 等)
        # ----------------------------------------------------------------------
        'Technology': 245,      # XLK - 科技類股ETF
        'Financials': 52,       # XLF - 金融類股ETF
        'Healthcare': 145,      # XLV - 醫療保健ETF
        'Consumer Disc': 228,   # XLY - 非必需消費品ETF
        'Comm Services': 105,   # XLC - 通訊服務ETF
        'Industrials': 142,     # XLI - 工業類股ETF
        'Consumer Staples': 82, # XLP - 必需消費品ETF
        'Energy': 90,           # XLE - 能源類股ETF
        'Utilities': 79,        # XLU - 公用事業ETF
        'Materials': 90,        # XLB - 原物料ETF
        'Real Estate': 43,      # XLRE - 房地產ETF

        # ----------------------------------------------------------------------
        # 外匯 (Forex)
        # 來源: Trading Economics
        # ----------------------------------------------------------------------
        'EUR/USD': 1.0420,      # 歐元/美元
        'USD/JPY': 156.00,      # 美元/日圓
        'GBP/USD': 1.2350,      # 英鎊/美元
        'USD/CNY': 7.28,        # 美元/人民幣
        'USD/TWD': 32.60,       # 美元/新台幣
        'DXY': 107.5,           # 美元指數

        # ----------------------------------------------------------------------
        # 原物料 (Commodities) - 單位: USD
        # 來源: JM Bullion, Trading Economics
        # ----------------------------------------------------------------------
        'Gold': 4997,           # 黃金現貨 (2026/01/24: $4,996.68/oz)
        'Silver': 62.5,         # 白銀現貨 (估計值，隨金價上漲)
        'WTI Crude': 75.0,      # WTI原油期貨
        'Brent': 79.0,          # 布蘭特原油期貨
        'Natural Gas': 3.90,    # 天然氣期貨
        'Copper': 4.30,         # 銅期貨

        # ----------------------------------------------------------------------
        # 加密貨幣 (Cryptocurrencies) - 單位: USD
        # ----------------------------------------------------------------------
        'Bitcoin': 105000,      # 比特幣
        'Ethereum': 3400,       # 以太幣

        # ----------------------------------------------------------------------
        # 波動率指數 (Volatility Indices)
        # 來源: CBOE
        # ----------------------------------------------------------------------
        'VIX': 16.0,            # VIX波動率指數 (2026/01/24: 開盤16.65, 低15.27)
        'MOVE': 95,             # MOVE債券波動率指數
    }
